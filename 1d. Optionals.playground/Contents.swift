...
// I am setting an optional Int value to .none (similar to nil)
let someOptional: Int? = .none

// Let's try to convert a String to an Int
//
// Using the String's Int() typecasting method, I am trying to convert a string to a numeric value. Since not
// all strings can be converted to an Integer, the Int() returns an optional, "Int?". This way
// Iam recognize failed conversions without having to trap exceptions or use other arcane
// methods to recognize the failure.
//
// Here's an optional in action

var notNumber = "notNumber"
var failedConversion = Int(notNumber)
